...
import { siteConfig as buildTimeConfig, type MesaSiteConfig } from '../config/site.config'
...
export const useSiteContentStore = defineStore('siteContent', () => {
  const config = ref<MesaSiteConfig>(buildTimeConfig)
  ...
  async function hydrate() {
    ...
    try {
      ...
      config.value = deepMerge(buildTimeConfig, res.content) as MesaSiteConfig
      hydrated.value = true
    } catch (e) {
